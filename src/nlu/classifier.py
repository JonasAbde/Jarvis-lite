...
class IntentClassifier:
    ...
    def _load_model(self) -> bool:
        ...
        try:
            ...
            self.vectorizer, self.classifier, self.intents = joblib.load(self.model_path)
            self.loaded = True
    ...
    def predict(self, text: str) -> Tuple[str, float]:
        ...
        try:
            ...
            if max_confidence < self.threshold:
                logger.info(f"Input '{text}' under tærskelværdi: {max_confidence:.2f} < {self.threshold}")
                return "unknown", max_confidence
                
            # Forudsig intent
            intent = self.classifier.predict(X)[0]
            logger.info(f"Klassificeret '{text}' som '{intent}' med konfidens {max_confidence:.2f}")
